...
const TestimonialCard = ({ quote, name, company, project, featured }) => (
  <article>
    ...
    >
      <Quote
        className={cn(
          "mb-4 text-primary/20",
          featured ? "w-10 h-10" : "w-7 h-7",
        )}
      />
      <p
        className={cn(
      ...
      <div>
        <p className="text-sm font-semibold text-txt">
          {name}
          {company && (
            <span className="text-txt-3 font-normal"> — {company}</span>
          )}
        </p>
        {project && <p className="text-xs text-primary/70 mt-0.5">{project}</p>}
      </div>
    </GlassCard>
  </article>
);
