...
const ProcessStep = ({ step, index }) => {
  ...
  return (
    <motion.div
      ref={ref}
      className="group grid grid-cols-1 md:grid-cols-[80px_1fr] gap-4 md:gap-8 py-8 sm:py-10 border-t border-ink/[0.06] first:border-t-0 relative"
    ...
    >
      ...
      <div className="relative">
        <span className="text-2xl sm:text-3xl font-bold text-txt-4 group-hover:text-txt-2 transition-colors duration-500">
          {step.number}
        </span>
        {/* Scroll-driven accent */}
        <motion.div
          className="absolute -left-3 top-0 w-px bg-ink/30 origin-top hidden md:block"
      ...
      <div className="max-w-xl">
        <h3 className="text-xl sm:text-2xl font-semibold text-txt mb-3 group-hover:text-gray-100 transition-colors duration-300">
          {step.title}
        </h3>
